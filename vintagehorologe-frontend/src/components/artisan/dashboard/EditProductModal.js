...
import { GiPocketWatch } from "react-icons/gi"; // Re-using the icon
...
const EditProductForm = () => {
  ...
  const [imagePreviews, setImagePreviews] = useState([]); // Previews for newly selected images
    ...
    const fetchProduct = async () => {
      try {
        ...
        setFormData({
          ...
          images: [], // Always start with empty new images array for edit
        });
        // Assuming product.imageData is an array of image URLs
        setExistingImages(product.imageData || []);
      } catch (error) {
        console.error("Error fetching product:", error);
        alert(error.response?.data?.message || "Failed to fetch product details.");
        navigate("/artisan/dashboard/products"); // Redirect if product not found or error
      }
    };
    fetchProduct();
  ...
  const validateForm = () => {
    ...
    // Only validate new images if they are being uploaded and total images exceed limit
    const totalImages = existingImages.length + formData.images.length;
    if (totalImages === 0) {
      newErrors.images = "At least one image is required";
    } else if (totalImages > 4) {
      newErrors.images = "Total images (existing + new) cannot exceed 4";
    }
  ...
  const handleImageChange = (e) => {
    ...
    if (oversizedFiles.length > 0) {
      imageErrors.images = "Each new image must be smaller than 5MB.";
    }

    // Check total images (existing + new)
    const currentTotalImages = existingImages.length + files.length;
    if (currentTotalImages > MAX_TOTAL_IMAGES) {
      imageErrors.images = `You can upload a maximum of ${MAX_TOTAL_IMAGES - existingImages.length} new images. Total images cannot exceed ${MAX_TOTAL_IMAGES}.`;
    }

    if (Object.keys(imageErrors).length > 0) {
      setErrors((prev) => ({ ...prev, ...imageErrors }));
      setImagePreviews([]); // Clear previews if there are errors
      setFormData((prev) => ({ ...prev, images: [] })); // Clear selected files
      return;
    }

    setFormData((prev) => ({ ...prev, images: files }));
    setImagePreviews(files.map((file) => URL.createObjectURL(file)));
    setErrors((prev) => ({ ...prev, images: "" })); // Clear previous image errors
  };
  ...
  const handleRemoveExistingImage = async (imageUrl) => {
    ...
    try {
      // Assuming your backend has an endpoint to remove specific images
      await axios.delete(`http://localhost:8080/api/products/${productId}/images`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        data: { imageUrlToRemove: imageUrl } // Send the URL of the image to remove
      });
    ...
    <div className="vintage-container py-5">
      <div className="horology-card shadow-lg">
        <div className="card-header horology-form-header">
          ...
          <h2 className="serif-font">Refine Horological Creation</h2>
          <p className="text-patina">Adjust Your Timeless Masterpiece Details</p>
        </div>
        ...
        <div className="card-body px-md-5 py-4">
          ...
          <form onSubmit={handleSubmit} encType="multipart/form-data">
              ...
              <div className="col-md-4">
                <div className="form-group">
                  ...
                  <input
                    ...
                    min="1"
                    max="10"
                    step="0.1" // Assuming it can be decimal like 3.5
            ...
            {existingImages.length > 0 && (
              <div className="form-group mb-4">
                <label className="text-patina">Existing Horological Documentation</label>
                <div className="gallery-grid existing-images mt-2">
            ...
            <div className="form-group mb-5">
              <label className="text-patina">Upload New Horological Documentation</label>
              <div className="vintage-upload">
                ...
                <div className="upload-label">
                  <GiPocketWatch className="me-2" />
                  Select New Certification & Gallery Images
                </div>
                {errors.images && <div className="invalid-feedback">{errors.images}</div>}
                <div className="upload-instructions">Maximum 4 Images (Total) • 5MB Each</div>
              </div>

              {imagePreviews.length > 0 && (
                <div className="gallery-grid new-previews mt-4">
                  {imagePreviews.map((src, index) => (
                    <div className="gallery-item" key={`new-preview-${index}`}>
                      <img src={src} alt={`New Preview ${index + 1}`} />
                      {/* No remove button here, user can re-select files if they make a mistake */}
                    </div>
                  ))}
                </div>
              )}
